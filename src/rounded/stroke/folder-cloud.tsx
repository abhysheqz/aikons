import React from "react";
const FolderCloud: React.FC<
  {
    size?: number | string | undefined,
  } & React.ComponentProps<"svg">
> = ({ size = 24, ...props }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      fill="none"
      viewBox="0 0 24 24"
      {...props}
    >
      <path
        stroke="currentColor"
        strokeLinecap="round"
        strokeWidth={1.5}
        d="M8 7.001h8.75c2.107 0 3.16 0 3.917.506.327.219.609.5.827.828.486.726.505 1.726.506 3.668v1M12 7l-.633-1.267c-.525-1.05-1.005-2.107-2.168-2.543C8.69 3 8.108 3 6.944 3c-1.816 0-2.724 0-3.406.38A3 3 0 0 0 2.38 4.54C2 5.22 2 6.129 2 7.946v3.056c0 4.716 0 7.073 1.464 8.538C4.705 20.782 6.587 20.971 10 21"
      />
      <path
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth={1.5}
        d="M13 18.667C13 19.955 14.007 21 15.25 21h4.725C21.093 21 22 20.06 22 18.9s-.917-2.1-2.035-2.1c.125-1.436-.985-2.8-2.465-2.8-1.294 0-2.357 1.03-2.466 2.344-1.141.113-2.034 1.11-2.034 2.323"
      />
    </svg>
  );
};
export default FolderCloud;
