import React from "react";
const Hexagon_01: React.FC<
  {
    size?: number | string | undefined,
  } & React.ComponentProps<"svg">
> = ({ size = 24, ...props }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      fill="none"
      viewBox="0 0 24 24"
      {...props}
    >
      <path
        fill="currentColor"
        d="M10.387 1.65c.541-.247 1.05-.4 1.613-.4s1.072.153 1.613.4c.52.237 1.117.586 1.85 1.013l2.83 1.648c.733.427 1.33.775 1.793 1.11.483.35.867.717 1.147 1.207.28.489.403 1.007.461 1.603.056.57.056 1.266.056 2.121v3.296c0 .855 0 1.55-.056 2.121-.058.596-.18 1.114-.46 1.603-.28.49-.665.858-1.148 1.207-.463.335-1.06.683-1.794 1.11l-2.83 1.648c-.732.427-1.33.776-1.849 1.013-.541.247-1.05.4-1.613.4s-1.072-.153-1.613-.4c-.52-.237-1.117-.586-1.85-1.013l-2.83-1.648c-.733-.427-1.33-.775-1.793-1.11-.483-.35-.867-.717-1.147-1.207-.28-.489-.403-1.007-.461-1.603-.056-.57-.056-1.266-.056-2.121v-3.296c0-.855 0-1.55.056-2.121.058-.596.18-1.114.46-1.603.28-.49.665-.858 1.148-1.207.463-.335 1.06-.683 1.794-1.11l2.83-1.648c.732-.427 1.33-.776 1.849-1.013"
      />
    </svg>
  );
};
export default Hexagon_01;
