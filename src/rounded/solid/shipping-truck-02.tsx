import React from "react";
const ShippingTruck_02: React.FC<
  {
    size?: number | string | undefined,
  } & React.ComponentProps<"svg">
> = ({ size = 24, ...props }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      fill="none"
      viewBox="0 0 24 24"
      {...props}
    >
      <path
        fill="currentColor"
        fillRule="evenodd"
        d="M12.06 3.25H5.1c-.64 0-1.214 0-1.679.062-.507.068-1.028.226-1.454.65-.427.424-.585.942-.654 1.447-.062.463-.062 1.034-.062 1.67v7.877c0 .42 0 .8.029 1.113.03.337.1.693.298 1.035.215.37.523.677.895.89.343.197 1.022.286 1.36.317a3.42 3.42 0 0 0 3.282 2.439 3.42 3.42 0 0 0 3.278-2.43h3.216a3.42 3.42 0 0 0 3.278 2.43 3.42 3.42 0 0 0 3.282-2.44c.338-.03 1.017-.119 1.36-.316.372-.213.68-.52.895-.89.198-.342.268-.698.298-1.035.029-.314.029-.692.029-1.113v-2.71c0-.325.001-.654-.089-.977s-.26-.605-.43-.883l-1.718-2.85c-.192-.318-.37-.612-.544-.847a2.4 2.4 0 0 0-.755-.693 2.4 2.4 0 0 0-.984-.287c-.293-.029-.638-.028-1.01-.028h-1.344c-.069-.505-.256-1.295-.683-1.72-.426-.423-.947-.581-1.454-.649-.465-.062-1.04-.062-1.679-.062m3.85 10.835q.466-.138.977-.14a3.42 3.42 0 0 1 3.276 2.42c.45 0 .633-.296.633-.476v-4.1l-2.543-4.164H15.91zM7.114 15.89c-.81 0-1.466.653-1.466 1.458 0 .806.657 1.459 1.466 1.459.81 0 1.466-.653 1.466-1.459 0-.805-.656-1.458-1.466-1.458m8.307 1.458c0-.805.656-1.458 1.466-1.458s1.466.653 1.466 1.458c0 .806-.656 1.459-1.466 1.459s-1.466-.653-1.466-1.459"
        clipRule="evenodd"
      />
    </svg>
  );
};
export default ShippingTruck_02;
