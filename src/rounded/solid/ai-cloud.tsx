import React from "react";
const AiCloud: React.FC<
  {
    size?: number | string | undefined,
  } & React.ComponentProps<"svg">
> = ({ size = 24, ...props }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      fill="none"
      viewBox="0 0 24 24"
      {...props}
    >
      <path
        fill="currentColor"
        d="M12 13.25a.75.75 0 0 1 .703.49l.258.697c.362.978.478 1.244.668 1.434s.456.306 1.434.668l.697.258a.75.75 0 0 1 0 1.406l-.697.258c-.978.362-1.244.478-1.434.668s-.306.456-.668 1.434l-.258.697a.75.75 0 0 1-1.406 0l-.258-.697c-.362-.978-.478-1.244-.668-1.434s-.456-.306-1.434-.668l-.697-.258a.75.75 0 0 1 0-1.406l.697-.258c.978-.362 1.244-.478 1.434-.668s.306-.456.668-1.434l.258-.697a.75.75 0 0 1 .703-.49"
      />
      <path
        fill="currentColor"
        d="M5.853 7.365A5.752 5.752 0 0 0 6.938 18.75a2 2 0 0 1 .868-3.126l.697-.258c.499-.184.811-.31.972-.384.074-.16.207-.48.391-.979l.258-.697a2 2 0 0 1 3.752 0l.258.697c.184.499.31.815.384.975.16.074.48.204.979.388l.697.258a2 2 0 0 1 .867 3.126h.439a5.25 5.25 0 0 0 .747-10.447 6.25 6.25 0 0 0-12.394-.938"
      />
    </svg>
  );
};
export default AiCloud;
